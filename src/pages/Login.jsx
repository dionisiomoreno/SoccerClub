...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    if (error) { toast.error('Credenziali non valide'); setLoading(false) }
    else navigate('/')
  }
    ...
    <div className="min-h-screen bg-[#f3f3f4] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#1ab394] text-white text-xl font-bold mb-3">SC</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                className="w-full border border-[#e7eaec] rounded px-3 py-2 text-[#676a6c] text-sm outline-none focus:border-[#1ab394]" placeholder="email@esempio.it"/>
            </div>
            <div>
              ...
              <div className="relative">
                <input type={show ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required
                  className="w-full border border-[#e7eaec] rounded px-3 py-2 pr-10 text-[#676a6c] text-sm outline-none focus:border-[#1ab394]" placeholder="••••••••"/>
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-[#1ab394] hover:bg-[#18a689] disabled:opacity-50 text-white font-semibold py-2 rounded text-sm">
