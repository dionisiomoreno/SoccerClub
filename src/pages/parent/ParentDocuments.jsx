...
export default function ParentDocuments() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from('youth_documents')
      .select('*')
      .eq('youth_player_id', parent.youth_players.id)
  ...
  async function upload() {
    ...
    const { error } = await supabase.from('youth_documents').insert([{
    ...
    <div className="space-y-5">
      <div className="border-b border-[#e7eaec] pb-4">
        ...
        <p className="text-sm text-[#999] mt-1">
          Certificati e documenti di {child?.nome} {child?.cognome}
        </p>
      </div>
      ...
      {expired > 0 && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded p-3 text-red-600 text-sm">
          <AlertTriangle size={16}/> <strong>{expired}</strong> documento/i scaduto/i — rinnovalo al più presto!
        </div>
      )}
      {expiringSoon > 0 && (
        <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded p-3 text-yellow-700 text-sm">
          <AlertTriangle size={16}/> <strong>{expiringSoon}</strong> documento/i in scadenza entro 30 giorni
        </div>
      )}
      ...
      <div className="bg-white border border-[#e7eaec] rounded shadow-sm p-5 space-y-4">
        ...
        <div
          ...
          className={clsx('border-2 border-dashed rounded p-6 text-center cursor-pointer transition-colors',
            dragging ? 'border-[#27ae60] bg-[#27ae60]/5' : 'border-[#e7eaec] hover:border-[#27ae60]/50')}>
          <Upload size={24} className="mx-auto text-[#999] mb-2"/>
          {file
            ? <p className="text-[#676a6c] text-sm font-medium">{file.name}</p>
            : <p className="text-[#999] text-sm">Trascina un file o clicca per selezionare<br/><span className="text-xs">PDF, JPG, PNG</span></p>}
        </div>
        <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={e => setFile(e.target.files[0])}/>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold text-[#999] uppercase tracking-wide mb-1">Tipo documento</label>
        ...
        <button onClick={upload} disabled={uploading || !file}
          className="w-full bg-[#27ae60] hover:bg-[#229954] disabled:opacity-50 text-white py-2 rounded text-sm font-semibold transition-colors">
          ...
          <div className="divide-y divide-[#e7eaec]">
            {docs.map(d => (
              <div key={d.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
                <div className="w-9 h-9 rounded-full bg-[#27ae60]/10 text-[#27ae60] flex items-center justify-center flex-shrink-0">
                  <FileText size={18}/>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[#2f4050] font-medium text-sm">{DOC_TYPES[d.tipo] || d.tipo}</div>
                  <div className="text-xs text-[#999] mt-0.5">
                    Caricato il {format(new Date(d.created_at), 'd MMM yyyy', { locale: it })}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <a href={d.file_url} target="_blank" rel="noreferrer"
                    className="p-1.5 text-[#999] hover:text-[#27ae60] transition-colors" title="Scarica">
                    <Download size={16}/>
                  </a>
                  <button onClick={() => deleteDoc(d)}
                    className="p-1.5 text-[#999] hover:text-red-500 transition-colors" title="Elimina">
                    <Trash2 size={16}/>
                  </button>
                </div>
              </div>
