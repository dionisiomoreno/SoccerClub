...
export default function SCChat() {
  ...
  async function initChats() {
    ...
    // Crea/carica chat per Prima Squadra e ogni categoria
    const chatDefs = [
      { nome: '⚽ Prima Squadra', tipo: 'gruppo', key: 'prima_squadra' },
      { nome: '🌐 Generale', tipo: 'gruppo', key: 'generale' },
      ...((cats || []).map(c => ({ nome: `${c.nome}`, tipo: 'gruppo', key: `cat_${c.id}`, colore: c.colore })))
    ]

    const result = []
    for (const def of chatDefs) {
      // Cerca chat esistente per nome
      let { data: existing } = await supabase.from('chats').select('*').eq('nome', def.nome).single()
      if (!existing) {
        const { data: created } = await supabase.from('chats').insert([{ tipo: def.tipo, nome: def.nome }]).select().single()
        existing = created
      }
      if (existing) {
        // Aggiungi utente come partecipante se non lo è già
        const { data: part } = await supabase.from('chat_participants').select('id')
          .eq('chat_id', existing.id).eq('user_id', profile.id).single()
        if (!part) {
  ...
  const ROLE_COLORS = {
    admin: 'text-red-500', mister: 'text-blue-500',
    segreteria: 'text-purple-500', player_paid: 'text-[#1ab394]',
    ...
    <div className="flex h-[calc(100vh-8rem)] bg-white border border-[#e7eaec] rounded shadow-sm overflow-hidden">
      ...
      <div className="w-64 flex-shrink-0 border-r border-[#e7eaec] flex flex-col">
        <div className="p-4 border-b border-[#e7eaec]">
          <h2 className="text-[#2f4050] font-bold text-sm">Chat</h2>
          ...
          {chats.map(chat => (
            <button key={chat.id} onClick={() => setActiveChat(chat)}
              ...
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                ...
                {chat.nome.substring(0, 1) === '⚽' ? '⚽' : chat.nome.substring(0, 1) === '🌐' ? '🌐' : <Users size={16}/>}
              </div>
              ...
              <div key={msg.id} className={clsx('flex gap-2', isMe ? 'flex-row-reverse' : 'flex-row')}>
                ...
                <div className={clsx('max-w-xs lg:max-w-md', isMe ? 'items-end' : 'items-start', 'flex flex-col')}>
                  ...
                  <div className={clsx('px-3 py-2 rounded-2xl text-sm',
                    isMe
                      ? 'bg-[#1ab394] text-white rounded-tr-sm'
                      : 'bg-gray-100 text-[#2f4050] rounded-tl-sm')}>
                    {msg.contenuto}
                  </div>
