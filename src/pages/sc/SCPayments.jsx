...
function generateRettaReceipt(scadenza, player, teamSettings) {
  const doc = new jsPDF()
  doc.setFillColor(26, 179, 148); doc.rect(0, 0, 210, 30, 'F')
  ...
  autoTable(doc, {
    ...
    body: [[`Retta mensile — ${MONTHS_FULL[scadenza.mese - 1]} ${scadenza.anno}`, `€${Number(scadenza.importo_pagato).toFixed(2)}`, scadenza.metodo_pagamento || '—']],
...
function generateReceipt(payment, player, receipt, teamSettings) {
  ...
  autoTable(doc, {
    startY: 76,
    head: [['Descrizione', 'Importo', 'Metodo']],
    body: [[`Quota ${payment?.tipo || ''} — ${MONTHS[(payment?.mese||1)-1]} ${payment?.anno || ''}`, `€${receipt.importo}`, receipt.metodo_pagamento || '—']],
    headStyles: { fillColor: [26,179,148] }, styles: { fontSize: 10 }
  })
  const y = doc.lastAutoTable.finalY + 15
  doc.setFillColor(245,245,245); doc.rect(14, y, 182, 20, 'F')
...
function PagaRettaModal({ scadenza, player, onClose, onSaved, teamSettings }) {
  ...
  async function save() {
    ...
    const { error } = await supabase.from('rette_scadenze').update({
      stato: 'pagato', data_pagamento: form.data_pagamento,
      metodo_pagamento: form.metodo_pagamento, numero_ricevuta: numRicevuta,
      importo_pagato: +form.importo_pagato, note: form.note,
      updated_at: new Date().toISOString()
    }).eq('id', scadenza.id)
    if (error) { toast.error(error.message); setLoading(false); return }
    const { data: ts } = await supabase.from('team_settings').select('club_id').single()
    await supabase.from('accounting_entries').insert([{
      club_id: ts?.club_id, data: form.data_pagamento, tipo: 'entrata', categoria: 'Quote SC',
      ...
      riferimento: numRicevuta, modulo: 'sc', note: form.note || null,
    }])
    generateRettaReceipt({ ...scadenza, ...form, numero_ricevuta: numRicevuta }, player, teamSettings)
      ...
      <div className="bg-white border border-[#e7eaec] rounded shadow-lg w-full max-w-sm">
        ...
        <div className="p-4 space-y-3">
          <div className="bg-[#1ab394]/5 border border-[#1ab394]/20 rounded p-3">
            ...
            <div className="text-xs text-[#999] mt-0.5">Retta {MONTHS_FULL[(scadenza.mese||1)-1]} {scadenza.anno} — dovuto: <strong>€{Number(scadenza.importo).toFixed(2)}</strong></div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-[#999] uppercase tracking-wide mb-1">Importo pagato (€)</label>
            <input type="number" min="0" step="0.01" value={form.importo_pagato}
          ...
          <div>
            ...
            <select value={form.metodo_pagamento} onChange={e => setForm(f => ({ ...f, metodo_pagamento: e.target.value }))}
              className="w-full border border-[#e7eaec] rounded px-3 py-2 text-[#676a6c] text-sm outline-none focus:border-[#1ab394]">
  ...
  async function save() {
    ...
    for (const id of selected) {
      ...
      await supabase.from('rette_scadenze').update({
        stato: 'pagato', data_pagamento: data, metodo_pagamento: metodo,
        numero_ricevuta: numRicevuta, importo_pagato: sc.importo, updated_at: new Date().toISOString()
      }).eq('id', id)
      await supabase.from('accounting_entries').insert([{
        club_id: ts?.club_id, data, tipo: 'entrata', categoria: 'Quote SC',
        descrizione: `Retta ${MONTHS_FULL[(sc.mese||1)-1]} ${sc.anno} — ${player?.cognome} ${player?.nome}`,
        importo: +sc.importo, metodo_pagamento: metodo, riferimento: numRicevuta, modulo: 'sc',
      }])
      ...
      <div className="bg-white border border-[#e7eaec] rounded shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
            ...
            {scadenze.map(sc => {
              ...
              return (
                <label key={sc.id} className="flex items-center gap-3 p-2 rounded hover:bg-gray-50 cursor-pointer">
                  <input type="checkbox" checked={selected.includes(sc.id)} onChange={() => toggle(sc.id)} className="accent-[#1ab394]"/>
                  <div className="flex-1"><span className="text-sm text-[#2f4050] font-medium">{p?.cognome} {p?.nome}</span></div>
          ...
          <div className="bg-[#1ab394]/5 border border-[#1ab394]/20 rounded p-3 flex justify-between text-sm">
            ...
            <strong className="text-[#1ab394]">€{scadenze.filter(s => selected.includes(s.id)).reduce((sum,s) => sum + +s.importo, 0).toFixed(2)}</strong>
          </div>
        </div>
        <div className="flex gap-2 p-4 border-t border-[#e7eaec]">
          <button onClick={onClose} className="flex-1 border border-[#e7eaec] hover:bg-gray-50 text-[#676a6c] py-2 rounded text-sm">Annulla</button>
      ...
      <div className="bg-white border border-[#e7eaec] rounded shadow-lg w-full max-w-sm">
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <input value={form.nome} onChange={e=>setForm(f=>({...f,nome:e.target.value}))} placeholder="Es. Quota mensile Pulcini"
              className="w-full border border-[#e7eaec] rounded px-3 py-2 text-[#676a6c] text-sm outline-none focus:border-[#1ab394]"/>
          </div>
          <div>
...
function PaymentModal({ players, categories, configs, onClose, onSaved }) {
  ...
  async function save() {
    ...
    toast.success('Pagamento registrato'); onSaved(); setLoading(false)
  }
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
...
function TabScadenziario({ players, categories, teamSettings }) {
  ...
  const loadScadenze = useCallback(async () => {
    setLoading(true)
    await supabase.from('rette_scadenze').update({ stato: 'scaduto' }).eq('stato', 'da_pagare').lt('data_scadenza', format(now, 'yyyy-MM-dd'))
    let q = supabase.from('rette_scadenze')
      .select('*, youth_players(id, nome, cognome, category_id, categories(nome,colore))')
      .eq('mese', filterMese).eq('anno', filterAnno).order('data_scadenza')
  ...
  async function generaScadenzeMese() {
    ...
    for (const cfg of configs) {
      ...
      const { error } = await supabase.from('rette_scadenze').insert([{
        retta_config_id: cfg.id, youth_player_id: cfg.youth_player_id,
        mese: filterMese, anno: filterAnno, importo: cfg.importo, data_scadenza: dataScad,
    ...
    <div className="space-y-4">
      {daPagare.length > 0 && (
        <div className="flex items-center justify-between bg-yellow-50 border border-yellow-200 rounded p-3">
          <div className="flex items-center gap-2 text-yellow-700 text-sm">
            <Bell size={16}/>
            <strong>{daPagare.length}</strong> rette da incassare •
            <strong>€{daPagare.reduce((s,r) => s + +r.importo, 0).toFixed(2)}</strong>
          </div>
      ...
      {isMeseCorrente && scadute.length > 0 && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded p-3 text-red-600 text-sm">
          <AlertTriangle size={15}/> <strong>{scadute.length}</strong> rette scadute non pagate
        </div>
      )}
      ...
      <div className="bg-white border border-[#e7eaec] rounded shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center text-[#999] py-10 text-sm">
            ...
            <br/>
            <button onClick={generaScadenzeMese} className="mt-2 text-[#1ab394] hover:underline text-xs">Genera le scadenze →</button>
          </div>
        ) : (
              ...
              {scadenze.map(sc => {
                ...
                return (
                  <tr key={sc.id} className="border-b border-[#e7eaec] hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-[#999] text-xs">{format(new Date(sc.data_scadenza), 'dd/MM/yyyy')}</td>
                    <td className="px-4 py-3"><span className="font-semibold text-[#2f4050]">€{Number(sc.importo).toFixed(2)}</span></td>
                    <td className="px-4 py-3">
                      ...
                      {sc.stato === 'pagato' && sc.data_pagamento && (
                        <div className="text-xs text-[#999] mt-0.5">{format(new Date(sc.data_pagamento), 'dd/MM/yyyy')} — {sc.metodo_pagamento}</div>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs font-mono text-[#999]">{sc.numero_ricevuta || '—'}</td>
                    <td className="px-4 py-3">
                      {sc.stato !== 'pagato' ? (
                        <button onClick={() => setPayModal({ scadenza: sc, player: p })}
                        ...
                        <button onClick={() => generateRettaReceipt(sc, p, teamSettings)}
      ...
      {payModal && (
        <PagaRettaModal scadenza={payModal.scadenza} player={payModal.player} teamSettings={teamSettings}
          onClose={() => setPayModal(null)} onSaved={() => { setPayModal(null); loadScadenze() }}/>
      )}
      {batchModal && (
        <PagaBatchModal scadenze={daPagare} players={players} teamSettings={teamSettings}
          onClose={() => setBatchModal(false)} onSaved={() => { setBatchModal(false); loadScadenze() }}/>
      )}
    </div>
  )
}
...
export default function SCPayments() {
  ...
  const [tab,           setTab]           = useState('scadenziario')
  ...
  useEffect(() => { loadBase(); loadRetteBadge() }, [])
  useEffect(() => { if (tab !== 'scadenziario') load() }, [tab, filterCat, filterStato, filterMese, filterAnno])
  ...
  async function loadBase() {
    ...
    setPlayers(pl||[]); setCategories(cat||[]); setConfigs(cfg||[]); setTeamSettings(ts)
  }

  async function loadRetteBadge() {
    const now = new Date()
    const { count } = await supabase.from('rette_scadenze').select('id', { count: 'exact' })
      .eq('mese', now.getMonth()+1).eq('anno', now.getFullYear()).eq('stato', 'da_pagare')
    ...
    <div className="space-y-4">
      <div className="border-b border-[#e7eaec] pb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {tab === 'payments' && <>
            <button onClick={()=>setConfigModal(true)} className="border border-[#e7eaec] hover:bg-gray-50 text-[#676a6c] px-3 py-2 rounded text-sm">+ Quota</button>
      ...
      {/* Contenuto tab */}
      {tab === 'scadenziario' && (
        <TabScadenziario players={players} categories={categories} teamSettings={teamSettings}/>
      )}
      ...
      {tab === 'configs' && (
        <div className="space-y-2">
          {configs.length === 0 ? (
            <div className="bg-white border border-[#e7eaec] rounded shadow-sm p-8 text-center text-[#999] text-sm">Nessuna quota configurata.</div>
          ) : configs.map(c=>(
            <div key={c.id} className="bg-white border border-[#e7eaec] rounded shadow-sm p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-[#999] mt-1">{c.tipo.charAt(0).toUpperCase()+c.tipo.slice(1)} — scadenza giorno {c.giorno_scadenza}</div>
              </div>
