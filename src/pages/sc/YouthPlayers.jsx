...
function PlayerModal({ player, categories, onClose, onSaved }) {
  ...
  async function loadRetta() {
    ...
    if (data) setRetta({ ...data, abilitata: data.active })
  }

  async function loadCategoryDefaults(catId) {
    if (!catId) return
  ...
  async function save() {
    ...
    try {
      if (isEdit) {
        ...
        const { data, error } = await supabase.from('youth_players')
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                ['luogo_nascita','Luogo nascita'], ['codice_fiscale','Codice fiscale'],
          ...
          <div className="border-t border-[#e7eaec] pt-4">
            ...
            {parents.map((p, idx) => (
              <div key={idx} className="mb-4">
                <div className="text-xs font-semibold text-[#1ab394] uppercase mb-2">
                  {p.tipo === 'padre' ? '👨 Padre' : '👩 Madre'}
                </div>
                <div className="grid grid-cols-2 gap-2">
...
function PlayerDetail({ player, categories, onClose, onEdit }) {
  ...
  useEffect(() => {
    ...
    supabase.from('rette_config').select('*').eq('youth_player_id', player.id).eq('active', true).maybeSingle()
      .then(({ data }) => setRettaD(data))
  }, [])
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-[#e7eaec] rounded shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          {/* Retta */}
          {retta && (
            <div className="bg-[#1ab394]/5 border border-[#1ab394]/20 rounded p-4">
              <h3 className="text-xs font-semibold text-[#999] uppercase tracking-wide mb-3">💶 Retta mensile</h3>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <div className="text-xs text-[#999]">Importo</div>
                  <div className="text-[#1ab394] font-bold">€{Number(retta.importo).toFixed(2)}/mese</div>
                </div>
                <div>
                  <div className="text-xs text-[#999]">Scadenza</div>
                  <div className="text-[#2f4050] font-medium">giorno {retta.giorno_scadenza}</div>
                </div>
                <div>
                  <div className="text-xs text-[#999]">Periodo</div>
                  <div className="text-[#2f4050] font-medium text-xs">
            ...
            <div className="bg-gray-50 rounded p-4">
              ...
              {parents.map(p => (
                <div key={p.id} className="mb-3 last:mb-0">
                  <div className="text-xs font-semibold text-[#1ab394] mb-1">
                    {p.tipo === 'padre' ? '👨 Padre' : '👩 Madre'}: {p.nome} {p.cognome}
                  </div>
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-[#e7eaec] hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {p.rette_config_active
                          ? <span className="text-[#1ab394] text-xs font-medium">💶 attiva</span>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setDetail(p)} className="text-[#999] hover:text-[#1c84c6]" title="Scheda">
                            <Eye size={15}/>
                          </button>
                          <button onClick={() => setModal(p)} className="text-[#999] hover:text-[#1c84c6]">
                            <Edit2 size={15}/>
                          </button>
                          <button onClick={() => deletePlayer(p)} className="text-[#999] hover:text-red-500">
                            <Trash2 size={15}/>
                          </button>
                        </div>
                      </td>
